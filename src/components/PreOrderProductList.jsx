...
import { fetchPreOrderProducts, deleteProductById, subscribeProducts } from '../utils/supabaseApi'
import { upsertCartItem } from '../utils/supabaseApi'
import { getCurrentUserId } from '../utils/supabaseAuth'
import './ProductList.css'

function PreOrderProductList({ onCartUpdate }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await fetchPreOrderProducts()
        setProducts(data)
      } catch (err) {
        console.error('載入預購商品失敗', err)
        alert('載入預購商品失敗，請稍後再試')
      }
    }
  ...
  const handleDelete = (id, name) => {
    if (window.confirm(`確定要刪除預購商品「${name}」嗎？`)) {
  ...
  return (
    <div className="product-list-container">
      <div className="list-header">
        <h2 className="page-title">預購商品</h2>
        {isAdmin && (
              ...
              <div className="product-card">
                ...
                <div className="product-card-info">
                  ...
                  <div className="product-price-row">
                    ...
                    <span className="product-stock">預購中</span>
                  </div>
                </div>
                {isAdmin && (
      ...
      <div className="stats">
        <p>共 {filteredProducts.length} 件預購商品</p>
      </div>
